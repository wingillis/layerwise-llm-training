...
def save_checkpoint(checkpoint_dir, step, model_data, optimizer_data, meta_data, rank=0):
    if rank == 0:
        ...
        model_path = Path(checkpoint_dir) / f"model_{step:06d}.pt"
        torch.save(model_data, model_path)
        logger.info(f"Saved model parameters to: {model_path}")
        # Save the metadata dict as json
        meta_path = Path(checkpoint_dir) / f"meta_{step:06d}.json"
...
def _resolve_checkpoint_path(checkpoints_dir, model_tag=None, step=None):
    ...
    if model_tag is None:
        ...
        model_tags = [f.name for f in checkpoints_path.iterdir() if f.is_dir()]
        if not model_tags:
    ...
    if step is None:
        ...
        checkpoint_files = list(Path(checkpoint_dir).glob("model_*.pt"))
        if not checkpoint_files:
            raise FileNotFoundError(f"No checkpoints found in {checkpoint_dir}")
        step = int(max(f.stem.split("_")[-1] for f in checkpoint_files))
